...
function _eventAdd(el, event, handler) {
 ...
 */
function _querySelector(selector, parent = document) {
 ...
 */
function _querySelectorAll(selector, parent = document) {
...
function _createElement(type, options = {}) {
...
const _prepend = (el, html, position) => {
...
const _append = (el, html, position) => {
...
function _hasClass(el, className) {
   if (!(el instanceof Element)) {
      throw new Error(
         "_hasClass: Provided element is not a valid DOM element."
      );
   }
 ...
 */
function _setAttribute(el, key, value) {
   if (!(el instanceof Element)) {
      throw new Error(
         "_setAttribute: Provided element is not a valid DOM element."
      );
   }
 ...
 */
function _removeAttribute(el, attributeName) {
   if (!(el instanceof Element)) {
      throw new Error(
         "_removeAttribute: Provided element is not a valid DOM element."
      );
   }
...
/****                               MATH                                      *****/
/**********************************************************************************/
function _getRandomInclusive(min, max) {
   return Math.floor(Math.random() * (max - min + 1)) + min;
}

function _getRandomExclusive(min, max) {
   if (max - min <= 2) {
...
function _summation(start, end, termFunction = (n) => n) {
   if (start > end) {
      throw new Error(
...
function _dateDifferenceInDays(date1, date2) {
 ...
 */
function _formatDate(date, { format = "YYYY-MM-DD", locale = "en-US" } = {}) {
 ...
 */
function _formatDateLocale(
   date,
   { locale = "en-US", dateStyle = "medium", timeStyle } = {}
) {
   if (!(date instanceof Date) || isNaN(date)) {
...
function _chunkArray(array, size) {
   ...
   if (typeof size !== "number" || size <= 0) {
      throw new Error(
         "Second argument to _chunkArray must be a positive integer."
      );
   }
...
function _ensureElements(el, throwOnError = true) {
   if (el instanceof Element) {
      ...
      if (el.every((item) => item instanceof Element)) {
         ...
         if (throwOnError) {
            throw new Error(
               "_ensureElements: Array contains non-element items."
            );
         } else {
            return [];
         }
      ...
      if (throwOnError) {
         throw new Error(
            "_ensureElements: Provided input is null or undefined."
         );
      } else {
         return [];
      }
